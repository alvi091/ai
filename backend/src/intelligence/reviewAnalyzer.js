...
const ASPECT_WORDS = {
  ...
  display: ['display', 'screen', 'panel', 'bright', 'oled', 'refresh rate'],
  comfort: ['comfort', 'fit', 'cushion', 'ear', 'head rest', 'long hours'],
  durability: ['durable', 'durability', 'sturdy', 'break', 'crack', 'torn', 'peel', 'rust'],
  performance: ['performance', 'fast', 'speed', 'lag', 'smooth', 'responsive', 'slow'],
  ...
  software: ['software', 'update', 'firmware', 'app', 'bug', 'crash', 'bluetooth', 'wifi'],
  heating: ['heat', 'heating', 'thermal', 'overheat', 'gets warm'],
  size: ['size', 'small', 'large', 'bulky', 'compact', 'slim', 'lightweight', 'heavy'],
...
function aspectHits(text) {
  ...
  for (const [key, words] of Object.entries(ASPECT_WORDS)) {
    for (const w of words) {
      ...
      if (re.test(t)) { hits.push(key); continue outer; }
    }
  }
  return hits;
}
...
function analyze(reviews = [], opts = {}) {
  ...
  const enriched = cleaned.map((r) => {
    ...
    const aspects = aspectHits(r.text);
